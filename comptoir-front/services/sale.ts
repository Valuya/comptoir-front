...
export class SaleService {
    ...
    closeSale(id:number, authToken:string):Promise<WsSaleRef> {
        ...
        return request
            .map(response=> {
                var saleRef = <WsSaleRef>response.json();
    ...
    reopenSale(id:number):Promise<WsSaleRef> {
    ...
    fromLocalConverter(localSale:LocalSale):WsSale {
        ...
        if (localSale.company != null) {
            sale.companyRef = new WsCompanyRef(localSale.company.id);
        }
        if (localSale.customer != null) {
            sale.customerRef = new WsCustomerRef(localSale.customer.id);
        }
